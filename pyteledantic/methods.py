...
@proxy_handler
def get_me(bot: Bot, session: Optional[requests.Session] = None, verify: bool = True) -> User:
    if not session:
        session = requests.Session()
    response = session.get(f'https://api.telegram.org/bot{bot.token}/getMe', verify=verify)
